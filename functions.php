...
if (!isset($_SESSION[$CONFIG_name.
'ipban
']) || (isset($_SESSION[$CONFIG_name.
'iptime
']) && (time() - $_SESSION[$CONFIG_name.
'iptime
']) > 300)) {
    include_once __DIR__ . 
'/ipban.php
';
    $_SESSION[$CONFIG_name.
'ipban
'] = ipban(); // ipban() function needs checking/updating for prepared statements
    $_SESSION[$CONFIG_name.
'iptime
'] = time();
}

if ($_SESSION[$CONFIG_name.
'ipban
'])
    die(
"Denied"
...
';
    if (!is_readable($filePath)) {
        error_log(
"Error reading castles file: "
 . $filePath);
        die(htmlformat(isset($lang[
'TXT_ERROR
']) ? $lang[
'TXT_ERROR
'] : 
...
; // Removed potentially problematic chars §*°ºª
    for ($i=0; $i<strlen($string); $i++) {
...
';
    if (!is_readable($filePath)) {
         error_log(
"Error reading password dictionary: "
 . $filePath);
...
"rt"
     ...
     if (!$handle) {
         error_log(
"Error opening password dictionary: "
 . $filePath);
...
']));
    ...
    $log_account = (int)$_SESSION[$CONFIG_name.
'account_id
'];
...
function online_count() {
    ...
    $query = 
"SELECT COUNT(1) FROM `char` WHERE online = '1'"
;
    $result = execute_query($query, 
'online_count'
, 0, 0); // No report needed?
...
'];
    ...
    // Updated to use prepared statements
    $query = 
"SELECT UNIX_TIMESTAMP(`lastlogin`), `unban_time`, `state` FROM `login` WHERE `last_ip` = ?"
;
    $result = execute_query($query, 
'check_ban'
, 0, 0, 
...
function execute_query($query, $source = 
'none.php
', $database = 0, $save_report = 1, $types = 
''
, $params = []) {
    global $mysql;
...
function add_query_entry($source, $log_query, $types = 
''
, $params = []) {
    global $CONFIG_name, $mysql;
    
    $log_account = isset($_SESSION[$CONFIG_name.
'account_id
']) ? (int)$_SESSION[$CONFIG_name.
'account_id
'] : 0;
    $log_ip = $_SERVER[
'REMOTE_ADDR
...
;
    ...
    $insert_query = 
"INSERT INTO `cp_querylog` (`Date`, `User`, `IP`, `page`, `Query`) VALUES(NOW(), ?, ?, ?, ?)"
;
...
, 1, 0, 
'isss'
, [$log_account, $log_ip, $source, $full_log]);
}
...
function server_status() {
    ...
    $query_check = 
"SELECT `last_checked`, `status`, TIMESTAMPDIFF(SECOND, `last_checked`, NOW()) FROM `cp_server_status` LIMIT 1"
;
...
, 1, 0);
    ...
    if ($result_check && $line = $result_check->fetch_row()) {
        ...
        $query_insert = 
"INSERT INTO `cp_server_status` (last_checked, status) VALUES(NOW(), 0)"
;
        execute_query($query_insert, 
"server_status_insert"
, 1, 0);
...
'connect_timeout'
 => 1]]); // 1 second timeout
        ...
        $acc_socket = @stream_socket_client(
"tcp://"
 . $CONFIG_accip . 
":"
 . $CONFIG_accport, $errno, $errstr, 1, STREAM_CLIENT_CONNECT, $context);
        if ($acc_socket) {
            $acc_online = true;
            fclose($acc_socket);
        }

        $char_socket = @stream_socket_client(
"tcp://"
 . $CONFIG_charip . 
":"
 . $CONFIG_charport, $errno, $errstr, 1, STREAM_CLIENT_CONNECT, $context);
        if ($char_socket) {
            $char_online = true;
            fclose($char_socket);
        }

        $map_socket = @stream_socket_client(
"tcp://"
 . $CONFIG_mapip . 
":"
 . $CONFIG_mapport, $errno, $errstr, 1, STREAM_CLIENT_CONNECT, $context);
        if ($map_socket) {
            $map_online = true;
            fclose($map_socket);
        }
        ...
        $query_update = 
"UPDATE `cp_server_status` SET last_checked = NOW(), status = ?"
;
...
"Status"
    ...
    echo 
"<tr><td><span style=\"cursor:pointer\" onMouseOver=\"this.style.color='#FF3300'\" onMouseOut=\"this.style.color='#000000'\" "

         . 
"onClick=\"return LINK_ajax('"
 . $safe_page . 
"','"
 . $safe_div . 
"')\"><b>"
...
; // Basic path safety
    if (!is_readable($filePath)) {
        error_log(
"Error reading mail definition file: "
 . $filePath);
        die(htmlformat(isset($lang[
'TXT_ERROR
']) ? $lang[
...
"rt"
     ...
     if (!$handle) {
         error_log(
"Error opening mail definition file: "
 . $filePath);
         die(htmlformat(isset($lang[
'TXT_ERROR
']) ? $lang[
...
'Saturday'
    ...
    if (empty($woe_times_str)) {
        echo 
"<tr><td colspan='3'>No WoE times configured.</td></tr>"
;
        return;
    }
...
', $period, $matches)) {
            $day_abbr = strtolower($matches[1]);
            $start_time = substr_replace($matches[2], 
':'
, 2, 0); // Format HH:MM
            $end_time = substr_replace($matches[3], 
':'
, 2, 0);   // Format HH:MM
            ...
            echo 
"<tr><td align=\"right\">".$day_name."</td><td>&nbsp;</td><td align=\"left\">".$start_time." - ".$end_time."</td></tr>"
;
        }
    }
}
